 ...
 */
public class UuidJdbcBehaviorEventPersisterImpl extends JdbcDaoSupport
				...
				try {
					...
					try {
						...
						for (BehaviorEvent next : events) {
							...
							insert.setString(1, ((UUID) next.getId()).toString());
							nullSafeSet(insert, 2, parent == null ? null : ((UUID) parent.getId()).toString(), Types.VARCHAR);
